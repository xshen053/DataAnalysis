...
from Visualization_second3 import calculate3
...
class ExampleFrame(wx.Frame):
    def __init__(self, parent):
        ...
        self.quote = wx.StaticText(self.panel, label="Your Input:")
        self.result = wx.StaticText(self.panel, label="")
        self.result.SetForegroundColour(wx.RED)
        self.button = wx.Button(self.panel, label="Calc")
        self.lblCode = wx.StaticText(self.panel, label="Fund Code:")
        self.editCode = wx.TextCtrl(self.panel, size=(140, -1))
        self.lblStartDate = wx.StaticText(self.panel, label="Start Date(YYYY-MM-DD):")
        self.editStartDate = wx.TextCtrl(self.panel, size=(140, -1))
        self.lblEndDate = wx.StaticText(self.panel, label="Start Date(YYYY-MM-DD):")
        self.editEndDate = wx.TextCtrl(self.panel, size=(140, -1))
        ...
        self.sizer.Add(self.lblCode, (1, 0))
        self.sizer.Add(self.editCode, (1, 1))
        self.sizer.Add(self.lblStartDate, (2, 0))
        self.sizer.Add(self.editStartDate, (2, 1))
        self.sizer.Add(self.lblEndDate, (3, 0))
        self.sizer.Add(self.editEndDate, (3, 1))
    ...
    def OnButton(self, e):
        # self.result.SetLabel(self.editCode.GetValue())
        self.result.SetLabel(self.editCode.GetValue() +" from:" + self.editStartDate.GetValue() +" to:" + self.editEndDate.GetValue())
        code = self.editCode.GetValue()
        start_time = self.editStartDate.GetValue()
        enddate = self.editEndDate.GetValue()
        ...
        high, low, maxrate = calculate(datalist)
        '''
        date_yield_max, high, low, maxrate = calculate3(code, datalist, start_time)
        print("最高点日期%s" % high)
        print("最低点日期%s" % low)
        print("最大回撤率:")
        print("%.2f%%" % (maxrate * 100))
